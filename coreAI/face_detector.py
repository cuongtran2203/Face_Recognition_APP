...
if __name__ == "__main__":
    ...
    # orig_img = cv2.imread("841-07202305en_Masterfile.jpg")
    cam=cv2.VideoCapture("rtsp://admin:ZTLBTF@192.168.1.4:554")
    face_test = FaceDetector("face_rec_models/ulffd_landmark.tflite",720,1280)
    while True:
        _,frame=cam.read()
        try:    
            orig_img=cv2.resize(frame,(1280,720))
            ...
            cv2.putText(orig_img,"FPS : {:.2f}".format(fps),(10,20),cv2.FONT_HERSHEY_DUPLEX,1,(255,0,14),1)
            print(orig_img.shape)
            cv2.imshow("camera", orig_img)
